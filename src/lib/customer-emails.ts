...
export async function sendOrderConfirmationEmail(
  ...
  if (!process.env.RESEND_API_KEY) {
    console.log('[CustomerEmail] RESEND_API_KEY not set — skipping order confirmation')
    console.log(`  To: ${data.customerEmail} | Total: $${data.orderTotal.toFixed(2)}`)
    return { success: true }
  }
                  ...
                  <td style="padding: 32px; background-color: #FAF8F5;">
                    ...
                    <table role="presentation" style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
                      ...
                      <tr>
                        ...
                        <td style="padding: 8px 0; font-size: 14px; color: #1A1A1A; text-align: right;">${data.shippingCost === 0 ? 'Free' : '$' + data.shippingCost.toFixed(2)}</td>
                      </tr>
...
export async function sendShippingConfirmationEmail(
  ...
  if (!process.env.RESEND_API_KEY) {
    console.log('[CustomerEmail] RESEND_API_KEY not set — skipping shipping confirmation')
    console.log(`  To: ${data.customerEmail} | Carrier: ${data.carrier} | Tracking: ${data.trackingNumber}`)
    return { success: true }
  }
